...
def main():
    ...
    PerformanceMeter(p, [t for t in p.TASKS.NAMES if t != '3ddet'])
    ...
    if args.run_mode != 'infer':
        ...
        for task in p.TASKS.NAMES:
            train_dataset = get_train_dataset(p, train_transforms, task_name=task)
            train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, drop_last=True)
            train_dataloader = get_train_dataloader(p, train_dataset, train_sampler)
            train_dataloaders.append(train_dataloader)

        affinity_dataloaders = []
        for task in p.TASKS.NAMES:
            affinity_dataset = get_train_dataset(p, train_transforms, task_name=task)
            affinity_sampler = torch.utils.data.distributed.DistributedSampler(affinity_dataset, drop_last=True)
            affinity_dataloader = get_train_dataloader(p, affinity_dataset, affinity_sampler)
            affinity_dataloaders.append(affinity_dataloader)
    ...
    if DEBUG_FLAG and args.local_rank == 0:
        ...
        if True:
            ...
            eval_test = {}
        if '3ddet' in eval_test.keys():
            eval_test.pop('3ddet') # remove verbose 3ddet metrics
        print(eval_test)
